...
export const getPoolInfo = async (
	...
	try {
		...
		const reserve0 = num.hexToDecimalString(
			num.toHex(reserves[0])
		);
		const reserve1 = num.hexToDecimalString(
			num.toHex(reserves[1])
		);

		const isTokenA0 = tokenA.address.includes(
			num.toHex(token0)
		);
		...
		return {
			...
			prices,
			shareOfPool: balanceOf
				? new Percent(balanceOf, totalSupply)
				: undefined,
...
export const approveTokenCallback = async (
	...
	try {
		...
		// );
		const approve0Call = token0Contract.populate(
			"approve",
			{
				spender: ROUTER_ADDRESS[APP_CHAIN_ID],
				amount:
		...
		const approve1Call = token1Contract.populate(
			"approve",
			{
				spender: ROUTER_ADDRESS[APP_CHAIN_ID],
				amount:
...
export const approveTokenLqCallback = async (
	...
	try {
		...
		const approve0Call = tokenContract.populate(
			"approve",
			{
				spender: ROUTER_ADDRESS[APP_CHAIN_ID],
...
export const removeLiquidityCallback = async (
  ...
  try {
		if (
			!account || !library || !removeAmount || !tokens
		)
			throw Error("Invalid params");
