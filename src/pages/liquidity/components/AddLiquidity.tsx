...
const AddLiquidity = () => {
	...
	const [isShowTokenModal, setIsShowTokenModal] =
		useState<boolean>(false);
	...
	const [independentField, setIndependentField] =
		useState<Field>(Field.INPUT);
	const web3State = useWeb3Store();

	const { data, isLoading: isLoadingPool } =
		useSWR<{
		...
		(value: string, independentField: Field) => {
			...
			const remainField =
				independentField === Field.INPUT
					? Field.OUTPUT
					: Field.INPUT;
			const decimalsIndependent =
				tokens?.[independentField]?.decimals ??
				18;
			const remainDecimals =
				tokens?.[remainField]?.decimals ?? 18;
			...
			if (
				...
				// Miss case noLiquidity
				const remainParsedAmount =
					parsedAmount.multiply(
						data?.poolInfo.prices[remainField]
							?.raw ?? "1"
					);
				setParsedTokenAmounts({
					[independentField]: new TokenAmount(
	...
	return (
		<div className="flex flex-col items-start">
			<div className="flex w-[500px] flex-col items-end justify-center bg-[#1D1C21] gap-8 p-8">
					...
					<div className="flex flex-col items-start gap-[10px] self-stretch">
						...
						<div className="flex justify-center items-center gap-[10px] p-2 self-stretch bg-[rgba(39,227,171,0.10)]">
							<Typography className="text-xs text-[#27E3AB] flex-1 font-medium font-['Montserrat'] leading-[18px]">
								...
								<br /> These tokens automatically
								earn fees proportional to your
								share of the pool, and can be
				...
				) : Number(tokenAmounts[Field.INPUT]) === 0 ? (
					<div className="flex justify-center items-center gap-2 self-stretch py-[18px] px-6 bg-[#737373] cursor-pointer">
						<Typography className="text-base font-bold">
							Invalid Pair
						</Typography>
			...
			<div className="flex py-3 px-8 justify-center items-center gap-1 self-stretch bg-[#1E302F]">
				<Typography className="text-xs font-medium text-[#27E3AB]">
					<b>Tip:</b> Smaller K, greater liquidity
					concentration. Read how to choose K
				</Typography>
				<ShareIcon />
			</div>
			{isShowTokenModal && (
