...
const SwapPage = () => {
	...
	const { account, address, isConnected } =
		useAccount();
	...
	const isDisableBtn: boolean = useMemo(() => {
		...
		if (
			BigInt(input) >
			BigInt(balances[0]?.raw.toString() ?? "0")
		) {
			return true;
		}
		return false;
		...
		<div className="flex w-[500px] flex-col items-end justify-center bg-[#1D1C21] gap-8 p-8">
					...
					items={[
						{
							...
							children: (
								<div className="flex flex-col items-start gap-3 self-stretch text-sm font-medium">
									<div className="flex justify-between items-center self-stretch">
										<Typography>
											Price impact
										</Typography>
			...
			{isShowConfirmSwap && (
				<ConfirmSwapModal
					...
					token0={tokens[Field.INPUT]}
					token1={tokens[Field.OUTPUT]}
					token0InputAmount={trade?.inputAmount.toSignificant(
						2
					)}
					token1OutputAmount={trade?.outputAmount.toSignificant(
						2
					)}
					trade={trade}
